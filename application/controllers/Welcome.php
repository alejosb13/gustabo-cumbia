...
class Welcome extends CI_Controller
{
	...
	{
		parent::__construct();
		// Cargar el modelo de noticias para todas las funciones
	...
	public function interna_noticias($id = null)
	{
		$data = array();
		$data["css"] = array(
		...
		// Si se proporciona un ID, obtener la noticia específica
		if ($id !== null) {
			$data["noticia"] = $this->Noticia_model->getNewById($id);

			// Si no se encuentra la noticia, redirigir a la lista
			if (!$data["noticia"]) {
				redirect(base_url("list_noticias"));
				return;
			}

			$data["section"] = $data["noticia"]->titulo;
		}
